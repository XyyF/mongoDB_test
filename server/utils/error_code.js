...
/** 该课程正在在线商城售卖 */
errorCode.TAPED_EDUCLASS_IS_ON_SHELF = defineCode(3156, '该课程正在在线商城售卖');

/** 不能删除已有报名记录的商品信息 */
errorCode.CAN_NOT_DEL_SOLD_COMMODITY = defineCode(3157, '不能删除已有报名记录的商品信息');

/** EXCEL单次最多导入两千条数据 */
errorCode.EXCEED_EXCEL_IMPORT_LIMIT = defineCode(3158, 'EXCEL单次最多导入两千条数据');

/** 私播课程名字重复 */
errorCode.TAPED_COURSE_NAME_REPEATED = defineCode(3159, '私播课程名字重复');

/** 私播课名字重复 */
errorCode.TAPED_EDUCLASS_NAME_REPEATED = defineCode(3160, '私播课名字重复');

/** 有未确认的购买请求，请确认以后再发起新的购买 */
errorCode.EXIST_NOT_CONFIRM_PURCHASE = defineCode(3161, '有未确认的购买请求，请确认以后再发起新的购买');

/** 当前版本仅支持免费商品的创建 */
errorCode.ONLY_CAN_CREATE_FREE_COURSE = defineCode(3162, '当前版本仅支持免费商品的创建');

/** 正式学员无法预约试听线上课程 */
errorCode.SIGNED_STUDENT_ALREADY_EXIST = defineCode(3163, '您已经是正式学员了哦，可以直接联系老师进行试听课');

/** 意向学员无法预约试听线上课程 */
errorCode.POTENTIAL_STUDENT_ALREADY_EXIST = defineCode(3164, '您已经成功预约试听了，机构教师会尽快和您确定试听时间');

/** 已经提交过注册信息 */
errorCode.ALREADY_SUBMITTED_REGISTER_INFO = defineCode(3165, '您已经成功提交了个人信息，机构管理员会尽快和您联系');

/** 剩余商品数量不足 */
errorCode.NOT_ENOUGH_COMMODITY = defineCode(3166, '剩余商品数量不足');

/** 目前仅支持免费商品购买 */
errorCode.CAN_ONLY_BUY_FREE_COMMODITY = defineCode(3167, '线上付费即将上线，现在请联系机构进行报名购买');

/** 已经注册过了，请直接登录 */
errorCode.ALREADY_REGISTERED = defineCode(3168, '当前手机号已经注册过了，请直接登录');

/** 当前学员剩余课时未消耗完，不能设置为结业状态 */
errorCode.TOO_MANY_REMAIN_PERIODS = defineCode(3169, '当前学员剩余课时未消耗完，不能设置为结业状态');

/** 订单状态错误 */
errorCode.ORDER_STATE_WRONG = defineCode(3170, '订单状态错误');

/** 商品名字重复 */
errorCode.COMMODITY_NAME_REPEATED = defineCode(3171, '商品名字重复');

/** 商城未启用 */
errorCode.MALL_NOT_ENABLE = defineCode(3172, '当前校区商城还未启用，需要机构管理员开启商城服务');

/** 未注册 */
errorCode.NOT_SIGNUP = defineCode(3173, '未注册，请先入学登记（若已经注册请直接登录）', false);

/** 指定类型微服务不存在 */
errorCode.MICRO_SERVER_NOT_EXIST = defineCode(3174, '指定类型微服务不存在');

/** 指定类型微服务没有可运行的实例 */
errorCode.MICRO_SERVER_NOT_RUNNING = defineCode(3175, '指定类型微服务没有可运行的实例');

/** 没有获取班级列表的权限 */
errorCode.NO_READ_EDUCLASS_AUTH = defineCode(3176, '当前管理员没有获取班级列表的权限');

/** 商城不能开启 */
errorCode.MALL_CAN_NOT_OPEN = defineCode(3177, '商城系统暂时处于内测阶段，详情请联系校精灵工作人员');

/** 没有获取教师统计权限 */
errorCode.NO_READ_TEACHER_STATS_AUTH = defineCode(3178, '当前管理员没有查看教师统计的权限');

/** 没有获取助教统计权限 */
errorCode.NO_READ_ASSISTANT_STATS_AUTH = defineCode(3179, '当前管理员没有查看助教统计的权限');

/** 没有获取咨询师统计权限 */
errorCode.NO_READ_COUNSELOR_STATS_AUTH = defineCode(3180, '当前管理员没有查看咨询师统计的权限');

/** 没有获取学管统计权限 */
errorCode.NO_READ_INSTRUCTOR_STATS_AUTH = defineCode(3181, '当前管理员没有查看学管统计的权限');

/** 同一校区下课程名重复 */
errorCode.EDUCATION_SUBJECT_REPEAT = defineCode(3200, '同一校区下课程名不能重复');

/** 同一校区下模板名重复 */
errorCode.EDUCATION_TEMPLATE_REPEAT = defineCode(3201, '同一校区下模板名不能重复');

/** 校区下的模板不存在 */
errorCode.EDUCATION_TEMPLATE_NOTEXIST = defineCode(3202, '当前启用的模板不存在');

/** 校区下没有启用的模板 */
errorCode.EDUCATION_TEMPLATE_NOTENABLE = defineCode(3203, '当前校区下没有启用的模板');

/** 校区下没有教研主管信息 */
errorCode.EDUCATION_NO_EDUCATIONMANAGER = defineCode(3204, '当前校区下没有教研主管信息');

/** 同一校区下活动名重复 */
errorCode.ACTIVITY_NAME_REPEAT = defineCode(3300, '同一校区下活动名不能重复');

/** 校区下不存在该活动 */
errorCode.ACTIVITY_NOT_EXIST = defineCode(3301, '当前校区下不存在该活动');

/** 活动报名未开始 */
errorCode.ACTIVITY_ENTRY_NOT_START = defineCode(3302, '活动报名未开始');

/** 活动报名已结束 */
errorCode.ACTIVITY_ENTRY_HAS_ENDED = defineCode(3303, '活动报名已结束');

/** 活动已结束 */
errorCode.ACTIVITY_HAS_ENDED = defineCode(3304, '活动已结束');

/** 活动名额已满 */
errorCode.ACTIVITY_HAS_FULL = defineCode(3305, '活动名额已满');

/** 活动未上架不能报名 */
errorCode.ACTIVITY_NOT_ENABLE = defineCode(3306, '活动未上架不能报名');

/** 重复报名 */
errorCode.ACTIVITY_SIGN_UP_REPEAT = defineCode(3307, '请勿重复报名');

/** 名字长度不能超过50个字符 */
errorCode.CAN_NOT_EXCEED_50_CHARS = defineCode(3308, '名字长度不能超过50个字符', false);

/** 已有学员报名的活动不能下架 */
errorCode.CAN_NOT_UNABLE_ACTIVITY = defineCode(3309, '不能下架该活动', false);

/** 已有学员报名的活动不能删除 */
errorCode.CAN_NOT_DELETE_ACTIVITY = defineCode(3310, '不能删除该活动', false);

/** 该活动已过期不能签到 */
errorCode.CAN_NOT_CHECK_IN_ACTIVITY = defineCode(3311, '不能在活动可签到时间范围外签到', false);

/** 不满足发放奖励条件 */
errorCode.CAN_NOT_GIVE_AWARD = defineCode(3312, '不满足发放奖励条件', false);

/** 不能重复发放奖励 */
errorCode.CAN_NOT_REPEAT_GIVE_AWARD = defineCode(3313, '不能重复发放奖励', false);
